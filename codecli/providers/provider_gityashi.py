...
class GitEinProvider(GitServiceProvider):
    ...
    def send_pullreq(self, head_repo, head_ref, base_repo, base_ref):
        ...
        url = "https://git.ysdev.xyz/%s/-/merge_requests/new?" % head_repo + urlencode(
            {
    ...
    def get_remote_repo_name(self, remote):
        ...
        _, _, reponame = repourl.partition('git.ysdev.xyz/')
        if not reponame:
            _, _, reponame = repourl.partition('git.ysdev.xyz:')
        return reponame
    ...
    def get_remote_repo_url(self, remote):
        ...
        assert re.match(r"^git@git.ysdev.xyz:.+\.git$", giturl) or re.match(
            r"^https://git.ysdev.xyz/.+\.git$", giturl
        ), ("This url do not look like a git.ysdev.xyz repo url: %s" % giturl)
    ...
    def get_repo_git_url(self, repo_name, login_user=''):
        ...
        return 'https://git.ysdev.xyz/%s.git' % repo_name

    def search_username(self):
        email = utils.get_user_email()
        payload = json.load(
            urlopen(
                "https://api.git.ysdev.xyz/search/users?" + urlencode(dict(q=email))
            )
        )
    ...
    def merge_config(self):
        ...
        if not email:
            ...
            if not email.endswith('@yashihq.com'):
                email = '%s@yashihq.com' % getuser()
            email = utils.ask(
                "Please enter your @yashihq.com email [%s]: " % email, default=email
            )
